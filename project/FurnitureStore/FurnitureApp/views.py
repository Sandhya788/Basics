...
from FunitureApp.forms import ReForm,ItemsForm,UsgForm,Rltype,Rlupd,Pfupd,Changepwd
from FunitureApp.models import Funiture,Furniturelist,User,Rolereq
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from django.core.mail import send_mail
from FunitureStore import settings

# Create your views here.


def home(r):
	w=Funiture.objects.filter(uid_id=r.user.id)
	t=Funiture.objects.all()
...
@login_required
def restlist(r):
	y=Funiture.objects.filter(uid_id=r.user.id)
	if r.method=="POST":
...
@login_required
def rstvw(r,a):
	s=Funiture.objects.get(id=a)
	return render(r,"app/restview.html",{'z':s})

@login_required
def ritems(re):
	st=list(Funiture.objects.filter(uid_id=re.user.id))
