...
from FurnitureApp1.forms import ReForm,ItemsForm,UsgForm,Rltype,Rlupd,Pfupd,Changepwd
from FurnitureApp1.models import Furniture,Restaurantlist,User,Rolereq
...
def home(r):
	w=Furniture.objects.filter(uid_id=r.user.id)
	t=Furniture.objects.all()
	s=Furniture.objects.all()#category
	g=None #product
...
def forders(r):
	z=Restaurantlist.objects.get(id=m)
	return render(r,'app/orders.html',{'s':z})
